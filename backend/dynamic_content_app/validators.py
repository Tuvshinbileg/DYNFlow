...
def validate_dynamic_content(content_type_name, data):
    ...
    """
    try:
        content_type = ContentType.objects.get(name=content_type_name, is_active=True)
    except ContentType.DoesNotExist:
        raise ValidationError(f"Content type '{content_type_name}' not found or not active")
        ...
        try:
            if field.field_type == 'number':
                ...
                if isinstance(field_value, bool):
                    ...
                    validated_data[field_name] = field_value.lower() in ['true', '1', 'yes']
                else:
                ...
                if field.choices and field_value not in field.choices:
                    errors[field_name] = f"{field.display_name} must be one of: {', '.join(field.choices)}"
                else:
                    validated_data[field_name] = field_value
